...
OPTIONS s_server_options[] = {
    ...
    {"key2", OPT_KEY2, 's',    "-Private Key file to use for servername if not in -cert2"},
    ...
    {"dcert_chain", OPT_DCERT_CHAIN, '<',  "second certificate chain file in PEM format"},
    ...
    {"ext_cache", OPT_EXT_CACHE, '-',  "Disable internal cache, setup and use external cache"},
    ...
    {"rev", OPT_REV, '-',  "act as a simple test server which just sends back with the received text reversed"},
    {"async", OPT_ASYNC, '-', "Operate in asynchronous mode"},
    {"ssl_config", OPT_SSL_CONFIG, 's',  "Configure SSL_CTX using the configuration 'val'"},
    {"split_send_frag", OPT_SPLIT_SEND_FRAG, 'n',  "Size used to split data for encrypt pipelines"},
    {"max_pipelines", OPT_MAX_PIPELINES, 'n',  "Maximum number of encrypt/decrypt pipelines to be used"},
    {"read_buf", OPT_READ_BUF, 'n',  "Default read buffer size to be used for connections"},
    OPT_S_OPTIONS,
    OPT_V_OPTIONS,
    OPT_X_OPTIONS,
...
# ifndef OPENSSL_NO_NEXTPROTONEG
    ...
    if (nocert == 0) {
        s_key = load_key(s_key_file, s_key_format, 0, pass, engine, "server certificate private key file");
        if (!s_key) {
            ERR_print_errors(bio_err);
            goto end;
        }
        ...
        if (tlsextcbp.servername) {
            s_key2 = load_key(s_key_file2, s_key_format, 0, pass, engine, "second server certificate private key file");
            if (!s_key2) {
                ERR_print_errors(bio_err);
                goto end;
            }
    ...
    if (s_dcert_file) {
        ...
        s_dkey = load_key(s_dkey_file, s_dkey_format,  0, dpass, engine, "second certificate private key file");
        if (!s_dkey) {
            ERR_print_errors(bio_err);
            goto end;
        }
    ...
    if (srp_verifier_file != NULL) {
        ...
        if ((ret =
             SRP_VBASE_init(srp_callback_parm.vb,  srp_verifier_file)) != SRP_NO_ERROR) {
            BIO_printf(bio_err,
